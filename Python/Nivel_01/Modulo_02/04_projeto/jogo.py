...
# Herói: controlado pelo usuário    (Derivado = herda tudo do personagem)
...
class Personagem:
    ...
    def atacar(self, alvo):
        # dano = self.__nivel * 2     # dano do ataque (está em personagem, porque ambos tem o mesmo ataque padrão)
        dano = random.randint(self.get_nivel() * 2, self.get_nivel() * 4)       # colocando o dano de ataque variável
        alvo.receber_ataque(dano)
        print(f"{self.get_nome()} atacou {alvo.get_nome()} e causou {dano} de dano!")

    def receber_ataque(self, dano):
        # self.__vida = self.__vida - dano      # é o mesmo que -=
        self.__vida -= dano
        if self.__vida < 0:
            self.__vida = 0
...
class Heroi(Personagem):
    ...
    def ataque_especial(self, alvo):
        # dano = self.get_nivel() * 5     # dano aumentado (ataque especial)
...
####
# Teste de classes e criação de heroi e inimigo
# heroi = Heroi(nome="Heroi", vida=100, nivel=5, habilidade="Super Força")
# print(heroi.exibir_detalhes())
# inimigo = Inimigo(nome="Morcego", vida=80, nivel=5, tipo="Voador")
